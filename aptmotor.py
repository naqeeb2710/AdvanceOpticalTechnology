...
def main():
    # Initialize Thorlabs_APT
    apt = Thorlabs_APT(verbose=True, event_dialog=True)
    ...
    # Choose a device to work with
    if devices:
        chosen_device = devices[0]  # Choosing the first device for demonstration purposes
        serial_number = chosen_device[2]
        # model_name, _, _ = apt.get_hw_info(serial_number)
        # print(f"Selected device: {model_name} (Serial Number: {serial_number})")

        # Initialize the chosen device
        # apt.init_hw_device(serial_number)

        # Move the motor
        apt.mot_move_absolute_ex(serial_number, 90.0, wait=True)

        # Get motor position
        position = apt.mot_get_position(serial_number)
        print(f"Current motor position: {position} degrees")

        # Move the motor continuously
        apt.mot_move_velocity(serial_number, 1)  # Move forward continuously

        # Stop the motor after 5 seconds (for demonstration)
        import time
        time.sleep(5)
        apt.mot_stop_profiled(serial_number)
